...
class IsaacAntPushEnv(ManagerBasedRLEnv):
    ...
    def step(self, action):
        """환경 스텝 - IsaacLab이 기대하는 텐서 형태로 전달"""
        try:
            # 1. 액션을 IsaacLab이 기대하는 텐서 형태로 변환
            safe_action = self._prepare_isaac_action(action)
    ...
    def _prepare_isaac_action(self, action):
        """IsaacLab이 기대하는 형태의 텐서로 액션 변환"""
        try:
            # numpy 배열로 먼저 변환
            ...
            # 차원 조정
            if action_np.ndim == 1:
                action_np = action_np.reshape(1, -1)  # (action_dim,) -> (1, action_dim)
            
            # 클리핑
            action_np = np.clip(action_np, -1.0, 1.0)
            
            # IsaacLab이 기대하는 텐서로 변환
            action_tensor = torch.from_numpy(action_np).to(
                device=self.device, 
                dtype=torch.float32
            )
