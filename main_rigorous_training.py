...
def main():
    ...
    try:
        ...
        print(f"  DPMM Birth Heuristic: max_k likelihood_k < tau_birth = {args.tau_birth}")
        print(f"  DPMM Merge Heuristic: distance_ij < tau_merge = {args.tau_merge}")
        print(f"  SAC Critic Loss: L_Q = E[(Q(s,a) - (r + gamma(min Q(s',a') - alpha log pi(a'|s'))))^2]")
        print(f"  SAC Policy Loss: L_pi = E[alpha log pi(a|s) - Q(s,a)]")
